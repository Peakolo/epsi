...
<body id="page-top">
    ...
    <div id="content" class="carre">
        <div name="bas" style="background-color:#456BD9;color:white;">
            <form name="compterendu" method="post" action="traitement_modif_CR.php?id=<?php echo $data; ?>">
                <h1>Rapport de ticket</h1>

                <!-- Matériaux Selection -->
                <label class="titre">Matériaux :</label>
                <select name="MAT_NOM" id="mat-select">
                    <option value="">Veuillez choisir le matériel</option>
                    <?php 
                        $nom = $con->query("SELECT * FROM matériaux");
                        while ($data = $nom->fetch()) { 
                    ?>
                        <option value="<?php echo $data['nom_materiaux']; ?>"><?php echo $data['nom_materiaux']; ?></option>
                    <?php 
                        } 
                    ?>
                </select>
                ...
                <textarea rows="5" cols="50" name="MOT_NOM" class="zone" required></textarea>
                <br>

                <!-- Urgent Selection -->
                <label class="titre">Urgent :</label>
                <select name="Urgent_NOM" id="urgent-select">
                    <option value="">Veuillez choisir le niveau d'urgence</option>
                    <?php 
                        $nom = $con->query("SELECT * FROM urgent");
                        while ($data = $nom->fetch()) { 
                    ?>
                        <option value="<?php echo $data['niveaux_risque']; ?>"><?php echo $data['niveaux_risque']; ?></option>
                    <?php 
                        } 
                    ?>
                </select>
