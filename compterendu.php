...
<html lang="en">
	...
	<title>Formulaire Rapport de Ticket</title>
</head>
				...
				<div id="content" class="carre">
					<div name="bas" style="background-color:#456BD9;color:white;">
						<form name="compterendu" method="post" action="traitement_CR.php?id=<?php echo $data; ?>">
                            ...
                            <select name="MAT_NOM" id="pet-select">
                                            <option value="">Veuillez choisir le matériel</option>
                                            <?php $nom = $con->query("SELECT * FROM matériaux");

                                            while ($data = $nom->fetch()) { ?>
                                                <option value="<?php echo $data['nom_materiaux']; ?>"><?php echo  $data['prenom_materiaux']; ?></option>
                                            <?php } ?>
                                        </select></br>
							
						
							<label class="titre">Motif</label><textarea rows="5" cols="50" name="MOT_NOM" class="zone"required ></textarea></br>
                            <label class="titre">Urgent :</label>

                            <select name="Urgent_NOM" id="pet-select">
                                
                            <option value="">Veuillez choisir le matériel</option>
                                            <?php $nom = $con->query("SELECT * FROM urgent");

                                            while ($data = $nom->fetch()) { ?>
                                                <option value="<?php echo $data['niveaux_risque']; ?>"><?php echo  $data['prenom_risque']; ?></option>
                                            <?php } ?>
                                        </select></br>                           
